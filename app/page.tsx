...
export default function HomePage() {
  ...
  // MOBILE DASHBOARD: fetch newest open needs
  useEffect(() => {
    const fetchNeeds = async () => {
      try {
        // IMPORTANT:
        // Your Browse Needs page uses APPROVED_OPEN, not OPEN.
        // So Home must match or it will always look empty.
        ...
        setRecentNeeds((data as RecentNeed[]) || []);
      } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="block md:hidden mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Happening Now</h2>
              ...
              {recentNeeds.map((n) => (
                <Link key={n.id} href={`/needs/${n.id}`}>
                  <div className="bg-white rounded-lg shadow p-4 active:scale-[0.99] transition">
                    ...
                    <div className="text-xs text-gray-500">Tap to view / claim</div>
                  </div>
                </Link>
              ))}
            </div>
